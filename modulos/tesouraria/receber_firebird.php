...
function normalizarColunasChave($colunasChave): array
{
...
function montarChaveAtivo($item, array $colunasChave): ?string
{
    ...
    $partes = [];
    foreach ($colunasChave as $coluna) {
...
{
    $colunasChave = normalizarColunasChave($colunasChave);
...
function garantirTabelaSnapshotAtivos(PDO $pdo): void
{
    $pdo->exec("
        CREATE TABLE IF NOT EXISTS sync_firebird_ativos_temp (
            sync_id VARCHAR(64) NOT NULL,
            tabela VARCHAR(80) NOT NULL,
            chave_sync VARCHAR(255) NOT NULL,
            criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
            PRIMARY KEY (sync_id, tabela, chave_sync),
...
function processarAtivosFirebird(PDO $pdo, array $dados, array $config): void
{
    ...
    if (!empty($config['coluna_data'])) {
        ...
        if (!$inicio || !$fim) {
            echo json_encode([
                "erro" => "Informe inicio e fim para verificar ativos de $tabelaFirebird."
            ]);
            exit;
        }
    ...
    foreach ($registros as $item) {
        $id = montarChaveAtivo($item, $colunasChave);
        if ($id !== null) {
            $ids[$id] = true;
        }
    }
    ...
    garantirControleExclusaoTabela($pdo, $nomeTabela, $colunasChave);

    if ($syncId) {
        garantirTabelaSnapshotAtivos($pdo);
        ...
        if (!empty($ids)) {
            $stmtSnapshot = $pdo->prepare("
                INSERT IGNORE INTO sync_firebird_ativos_temp (sync_id, tabela, chave_sync)
                VALUES (?, ?, ?)
            ");
            foreach (array_keys($ids) as $id) {
                $stmtSnapshot->execute([$syncId, $nomeTabela, $id]);
            }
        }
        ...
        try {
            $stmtReativar = $pdo->prepare("
                UPDATE `$nomeTabela` m
                INNER JOIN sync_firebird_ativos_temp t
                    ON t.sync_id = ?
                   AND t.tabela = ?
                   AND $exprChave = t.chave_sync
                ...
                WHERE 1=1 $filtroSql
            ");
            $stmtReativar->execute(array_merge([$syncId, $nomeTabela], $filtroParams));
            $reativados = $stmtReativar->rowCount();

            $stmtExcluir = $pdo->prepare("
                UPDATE `$nomeTabela` m
                LEFT JOIN sync_firebird_ativos_temp t
                    ON t.sync_id = ?
                   AND t.tabela = ?
                   AND $exprChave = t.chave_sync
    ...
    try {
        ...
        $stmtReativar = $pdo->prepare("
            ...
            INNER JOIN tmp_firebird_ativos_sync t
                ON " . montarExpressaoChaveSql('m', $colunasChave) . " = t.id
            SET m.excluido_firebird = 'N',
                m.data_exclusao_firebird = NULL,
                m.motivo_sync = NULL,
                m.ultima_presenca_firebird = NOW()
...
elseif ($tabela === 'cr001') {
    ...
    $sql = "
        ...
        ON DUPLICATE KEY UPDATE
            ...
            TIPODOCORIGEM = VALUES(TIPODOCORIGEM),
            NUMDOCORIGEM = VALUES(NUMDOCORIGEM),
            TIPOCR = VALUES(TIPOCR),
            REGSTAMP = VALUES(REGSTAMP),
            USERLANC = VALUES(USERLANC),
            TIPOES = VALUES(TIPOES),
            DTLANC = VALUES(DTLANC),
            CHAVEINTEGRACAO = VALUES(CHAVEINTEGRACAO),
            excluido_firebird = 'N',
...
elseif ($tabela === 'est004') {
    ...
    $sql = "
        ...
        ON DUPLICATE KEY UPDATE
            ...
            PRECOPRAZO1 = VALUES(PRECOPRAZO1),
            PRECOCOMPRA = VALUES(PRECOCOMPRA),
            MEDIACOMPRA = VALUES(MEDIACOMPRA),
            MEDIACUSTO = VALUES(MEDIACUSTO),
            PRECOFINAL = VALUES(PRECOFINAL),
            MEDIAFINAL = VALUES(MEDIAFINAL),
            MLUCRO1 = VALUES(MLUCRO1),
            PVENDA1 = VALUES(PVENDA1),
            MLUCRO2 = VALUES(MLUCRO2),
            PVENDA2 = VALUES(PVENDA2),
            ESTOQUE = VALUES(ESTOQUE),
            ESTINICIAL = VALUES(ESTINICIAL),
            ESTMINIMO = VALUES(ESTMINIMO),
            ESTMAXIMO = VALUES(ESTMAXIMO),
            DTULTCOMPRA = VALUES(DTULTCOMPRA),
            DOCCOMPRA = VALUES(DOCCOMPRA),
            ULTFORNECEDOR = VALUES(ULTFORNECEDOR),
            DTULTVENDA = VALUES(DTULTVENDA),
            DOCVENDA = VALUES(DOCVENDA),
            ULTCLIENTE = VALUES(ULTCLIENTE),
            ULTQTDECOMPRADA = VALUES(ULTQTDECOMPRADA),
            INATIVO = VALUES(INATIVO),
            NOMEFISCAL = VALUES(NOMEFISCAL),
            PVENDA1ANT = VALUES(PVENDA1ANT),
            USERALTPRECO = VALUES(USERALTPRECO),
            REGSTAMP = VALUES(REGSTAMP),
...
elseif ($tabela === 'est007') {
    ...
    $sql = "
        ...
        ON DUPLICATE KEY UPDATE
            ...
            DTVENDA = VALUES(DTVENDA),
            NUMDOC = VALUES(NUMDOC),
            TIPOPAGTO = VALUES(TIPOPAGTO),
            CLIENTE = VALUES(CLIENTE),
            VENDEDOR = VALUES(VENDEDOR),
            OBSERVACAO = VALUES(OBSERVACAO),
            TOTVENDA = VALUES(TOTVENDA),
            TOTACRESCIMO = VALUES(TOTACRESCIMO),
            TOTDESCONTO = VALUES(TOTDESCONTO),
            TOTGERAL = VALUES(TOTGERAL),
            TOTIPI = VALUES(TOTIPI),
            TOTCUSTO = VALUES(TOTCUSTO),
            QTDEVENDA = VALUES(QTDEVENDA),
            TOTPRODVENDA = VALUES(TOTPRODVENDA),
            NUMPARCELAS = VALUES(NUMPARCELAS),
            CMCONTADOR = VALUES(CMCONTADOR),
            QTDEVENDA2 = VALUES(QTDEVENDA2),
            TIPOVENDA = VALUES(TIPOVENDA),
            CANCELADO = VALUES(CANCELADO),
            DTCANCELADO = VALUES(DTCANCELADO),
            FINANCGERADO = VALUES(FINANCGERADO),
            NUMCUPOM = VALUES(NUMCUPOM),
            USERDIGITA = VALUES(USERDIGITA),
            DTENTREGAPED = VALUES(DTENTREGAPED),
            USERLANC = VALUES(USERLANC),
