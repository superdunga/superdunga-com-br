...
$paginasOperacionais = ['painel','rotinas','fechamento','cobranca','mensagem','envios','historico','configuracoes'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($acao === 'salvar_config') {
        ...
        } elseif ($acao === 'salvar_cobranca_automacao') {
            $intervalo = max(10, min(3600, (int)($_POST['cobranca_intervalo_segundos'] ?? 30)));
            $stmt = $pdo_master->prepare("UPDATE whatsapp_operacional_config SET cobranca_ativo=?,cobranca_horario='10:00:00',cobranca_intervalo_segundos=? WHERE empresa_id=?");
            $stmt->execute([($_POST['cobranca_ativo'] ?? 'N') === 'S' ? 'S' : 'N',$intervalo,$empresaId]);
            if ($stmt->rowCount() === 0 && !whatsappOperacionalConfig($pdo_master, $empresaId)) {
                throw new Exception('Salve primeiro a configuracao da instancia operacional.');
            }
            $alerta = 'Automacao da cobranca semanal salva.';
        ...
        } elseif ($acao === 'preparar_cobranca_agora') {
            $configAtual = whatsappOperacionalConfig($pdo_master, $empresaId);
            if (!$configAtual) {
                throw new Exception('Configure primeiro a instancia operacional.');
            }
            ...
            $referencia = new DateTimeImmutable('now');
            $criados = whatsappOperacionalPrepararFilaCobranca($pdo_master, $configAtual, $referencia);
            $alerta = 'Fila de teste preparada: ' . $criados . ' novo(s) cliente(s).';
        } elseif ($acao === 'testar') {
            $configExistente = whatsappOperacionalConfig($pdo_master, $empresaId);
...
if ($paginaOperacional === 'cobranca') { $sqlFila .= " AND q.rotina_codigo='cobranca_semanal_clientes'"; }
...
$proximaCobranca = $config ? whatsappOperacionalProximaSegunda((string)($config['cobranca_horario'] ?? '10:00:00')) : null;
$stmt = $pdo_master->prepare("
    SELECT COUNT(*) total,
           SUM(CASE WHEN COALESCE(NULLIF(TRIM(c.CELULAR),''),'')='' THEN 1 ELSE 0 END) sem_celular,
           SUM(CASE WHEN EXISTS (SELECT 1 FROM armazem_cr001 cr WHERE cr.EMPRESA=ca.empresa_id AND cr.CLICONTADOR=ca.clicontador AND cr.CMCONTADOR=9 AND DATE(cr.DTVENC)<CURDATE() AND (cr.STATUS IS NULL OR cr.STATUS<>'QT') AND COALESCE(cr.VLRRESTANTE,0)>0 AND COALESCE(cr.excluido_firebird,'N')<>'S') THEN 1 ELSE 0 END) em_atraso
    FROM financeiro_clientes_cobranca_automatica ca
    LEFT JOIN armazem_cr002 c ON c.EMPRESA=ca.empresa_id AND c.CLICONTADOR=ca.clicontador
    WHERE ca.empresa_id=? AND ca.ativo='S'
");
$stmt->execute([$empresaId]);
$resumoCobranca=$stmt->fetch(PDO::FETCH_ASSOC) ?: ['total'=>0,'sem_celular'=>0,'em_atraso'=>0];
...
<?php elseif ($paginaOperacional === 'cobranca'): ?>
<div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-2 mb-3">
    <div><div class="small text-muted"><a href="rotinas.php" class="text-decoration-none">Rotinas</a> / Cobranca semanal de clientes</div><h2 class="h4 mb-0">Cobranca semanal de clientes</h2></div>
    <a href="rotinas.php" class="btn btn-outline-secondary btn-sm">Voltar para rotinas</a>
</div>
<ul class="nav nav-tabs flex-nowrap overflow-auto mb-3">
    <li class="nav-item"><a class="nav-link text-nowrap <?= $secaoRotina==='execucao'?'active':'' ?>" href="cobranca_automatica.php?secao=execucao">Execucao</a></li>
    <li class="nav-item"><a class="nav-link text-nowrap <?= $secaoRotina==='fila'?'active':'' ?>" href="cobranca_automatica.php?secao=fila">Fila</a></li>
    <li class="nav-item"><a class="nav-link text-nowrap <?= $secaoRotina==='historico'?'active':'' ?>" href="cobranca_automatica.php?secao=historico">Historico</a></li>
    <li class="nav-item"><a class="nav-link text-nowrap <?= $secaoRotina==='configuracao'?'active':'' ?>" href="cobranca_automatica.php?secao=configuracao">Configuracao</a></li>
</ul>
...
<div class="row g-3 mb-3">
    <div class="col-xl-3 col-md-6"><div class="card shadow-sm h-100"><div class="card-body"><div class="text-muted small">Instancia operacional</div><div class="fs-5 fw-semibold mt-1"><?= htmlspecialchars($config['instancia']??'Nao configurada') ?></div><div class="small <?= ($config['ativo']??'N')==='S'?'text-success':'text-warning' ?>"><?= ($config['ativo']??'N')==='S'?'Ativa':'Inativa' ?></div></div></div></div>
    <div class="col-xl-3 col-md-6"><div class="card shadow-sm h-100"><div class="card-body"><div class="text-muted small">Rotinas ativas</div><div class="display-6 fw-semibold"><?= (($config['fechamento_ativo']??'N')==='S'?1:0)+(($config['cobranca_ativo']??'N')==='S'?1:0) ?></div><div class="small text-muted">de 2 rotinas cadastradas</div></div></div></div>
    <div class="col-xl-3 col-md-6"><div class="card shadow-sm h-100"><div class="card-body"><div class="text-muted small">Sincronizacao CR001</div><div class="fs-5 fw-semibold mt-2 <?= $sincronizacaoFechamentoAtualizada?'text-success':'text-warning' ?>"><?= $sincronizacaoFechamentoAtualizada?'Atualizada':'Aguardando' ?></div><div class="small text-muted">Base usada nas duas rotinas</div></div></div></div>
...
<div class="card shadow-sm mb-3"><div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-2"><h2 class="h5 mb-0">Rotinas operacionais</h2><a href="rotinas.php" class="btn btn-primary btn-sm">Abrir rotinas</a></div><div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th>Rotina</th><th>Publico</th><th>Proxima execucao</th><th>Status</th><th class="text-end">Acao</th></tr></thead><tbody><tr><td class="fw-semibold">Fechamento mensal de clientes</td><td><?= max(0,$totalClientesFechamento-$semCelularFechamento) ?> apto(s)</td><td><?= $proximoEnvioAutomacao?htmlspecialchars($proximoEnvioAutomacao->format('d/m/Y H:i')):'-' ?></td><td><span class="badge <?= ($config['fechamento_ativo']??'N')==='S'?'text-bg-success':'text-bg-secondary' ?>"><?= ($config['fechamento_ativo']??'N')==='S'?'Ativa':'Inativa' ?></span></td><td class="text-end"><a href="fechamento_mensal.php" class="btn btn-sm btn-outline-primary">Abrir</a></td></tr><tr><td class="fw-semibold">Cobranca semanal de clientes</td><td><?= (int)$resumoCobranca['em_atraso'] ?> em atraso</td><td><?= $proximaCobranca?htmlspecialchars($proximaCobranca->format('d/m/Y H:i')):'-' ?></td><td><span class="badge <?= ($config['cobranca_ativo']??'N')==='S'?'text-bg-success':'text-bg-secondary' ?>"><?= ($config['cobranca_ativo']??'N')==='S'?'Ativa':'Inativa' ?></span></td><td class="text-end"><a href="cobranca_automatica.php" class="btn btn-sm btn-outline-primary">Abrir</a></td></tr></tbody></table></div></div>
...
<?php if ($paginaOperacional === 'cobranca' && $secaoRotina === 'execucao'): ?>
<div class="row g-3 mb-3">
    <div class="col-md-4"><div class="card shadow-sm h-100"><div class="card-body"><div class="text-muted small">Clientes marcados</div><div class="display-6 fw-semibold"><?= (int)$resumoCobranca['total'] ?></div><div class="small text-muted">Cadastro independente do fechamento mensal</div></div></div></div>
    <div class="col-md-4"><div class="card shadow-sm h-100"><div class="card-body"><div class="text-muted small">Marcados ainda em atraso</div><div class="display-6 fw-semibold"><?= (int)$resumoCobranca['em_atraso'] ?></div><div class="small text-muted">Revalidado no momento do envio</div></div></div></div>
    <div class="col-md-4"><div class="card shadow-sm h-100"><div class="card-body"><div class="text-muted small">Proxima execucao</div><div class="fs-5 fw-semibold mt-2"><?= ($config['cobranca_ativo']??'N')==='S' && $proximaCobranca ? htmlspecialchars($proximaCobranca->format('d/m/Y H:i')) : 'Automacao inativa' ?></div><div class="small text-muted">Segundas-feiras, as 10:00</div></div></div></div>
</div>
<div class="card shadow-sm"><div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-2"><h2 class="h5 mb-0">Cobranca automatica</h2><a href="../financeiro/clientes_cobranca_automatica.php" class="btn btn-sm btn-outline-primary">Administrar clientes</a></div><div class="card-body"><p class="mb-2">Toda segunda-feira, o sistema seleciona os clientes marcados que ainda possuam titulo vencido e envia um PDF com todos os titulos em aberto.</p><div class="small text-muted">Clientes sem celular ou sem atraso no momento do processamento sao ignorados com o motivo registrado na fila. Cada semana possui uma referencia unica para impedir envios duplicados.</div></div></div>
<?php endif; ?>

<?php if ($paginaOperacional === 'cobranca' && $secaoRotina === 'configuracao'): ?>
<div class="card shadow-sm"><div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-2"><h2 class="h5 mb-0">Automacao da cobranca semanal</h2><span class="badge <?= ($config['cobranca_ativo']??'N')==='S'?'text-bg-success':'text-bg-secondary' ?>"><?= ($config['cobranca_ativo']??'N')==='S'?'Ativa':'Inativa' ?></span></div><div class="card-body">
    <div class="alert alert-info py-2">Execucao fixa toda <strong>segunda-feira, as 10:00</strong>. Proxima execucao: <strong><?= $proximaCobranca?htmlspecialchars($proximaCobranca->format('d/m/Y H:i')):'-' ?></strong>.</div>
    <?php if (!$sincronizacaoFechamentoAtualizada): ?><div class="alert alert-warning py-2">A rotina aguardara uma sincronizacao CR001 concluida antes de preparar ou enviar as cobrancas.</div><?php endif; ?>
    <form method="post" class="row g-3 align-items-end"><input type="hidden" name="csrf" value="<?= htmlspecialchars($csrf) ?>"><input type="hidden" name="acao" value="salvar_cobranca_automacao"><div class="col-md-4"><label class="form-label">Status</label><select name="cobranca_ativo" class="form-select"><option value="S" <?= ($config['cobranca_ativo']??'N')==='S'?'selected':'' ?>>Ativa</option><option value="N" <?= ($config['cobranca_ativo']??'N')!=='S'?'selected':'' ?>>Inativa</option></select></div><div class="col-md-4"><label class="form-label">Intervalo entre clientes (seg.)</label><input type="number" min="10" max="3600" name="cobranca_intervalo_segundos" class="form-control" value="<?= (int)($config['cobranca_intervalo_segundos']??30) ?>" required></div><div class="col-md-4"><button class="btn btn-primary w-100">Salvar automacao</button></div></form>
</div></div>
<?php endif; ?>

<?php if ($paginaOperacional === 'envios' || (in_array($paginaOperacional,['fechamento','cobranca'],true) && $secaoRotina === 'fila')): ?>
<div class="card shadow-sm mb-3">
    <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-2"><h2 class="h5 mb-0"><?= $paginaOperacional==='fechamento'?'Fila do fechamento mensal':($paginaOperacional==='cobranca'?'Fila da cobranca semanal':'Fila operacional') ?></h2><div class="d-flex gap-2"><?php if($paginaOperacional==='fechamento'): ?><form method="post" onsubmit="return confirm('Preparar agora a fila da competencia anterior?')"><input type="hidden" name="csrf" value="<?= htmlspecialchars($csrf) ?>"><input type="hidden" name="acao" value="preparar_fila_agora"><button class="btn btn-sm btn-outline-primary">Preparar fila agora</button></form><?php elseif($paginaOperacional==='cobranca'): ?><form method="post" onsubmit="return confirm('Preparar agora uma fila de teste para os clientes marcados?')"><input type="hidden" name="csrf" value="<?= htmlspecialchars($csrf) ?>"><input type="hidden" name="acao" value="preparar_cobranca_agora"><button class="btn btn-sm btn-outline-primary">Preparar fila agora</button></form><?php endif; ?><form method="post"><input type="hidden" name="csrf" value="<?= htmlspecialchars($csrf) ?>"><input type="hidden" name="acao" value="reprocessar_erros"><?php if($paginaOperacional==='fechamento'): ?><input type="hidden" name="rotina_codigo" value="fechamento_mensal_clientes"><?php elseif($paginaOperacional==='cobranca'): ?><input type="hidden" name="rotina_codigo" value="cobranca_semanal_clientes"><?php endif; ?><button class="btn btn-sm btn-outline-warning">Reprocessar erros</button></form><form method="post" onsubmit="return confirm('Cancelar todos os envios ainda pendentes?')"><input type="hidden" name="csrf" value="<?= htmlspecialchars($csrf) ?>"><input type="hidden" name="acao" value="cancelar_fila"><?php if($paginaOperacional==='fechamento'): ?><input type="hidden" name="rotina_codigo" value="fechamento_mensal_clientes"><?php elseif($paginaOperacional==='cobranca'): ?><input type="hidden" name="rotina_codigo" value="cobranca_semanal_clientes"><?php endif; ?><button class="btn btn-sm btn-outline-danger">Cancelar pendentes</button></form></div></div>
    <?php if ($paginaOperacional === 'envios'): ?><div class="card-body border-bottom"><form method="get" class="row g-2 align-items-end"><div class="col-lg-4"><label class="form-label">Rotina</label><select name="rotina" class="form-select"><option value="">Todas</option><option value="fechamento_mensal_clientes" <?= $filtroRotina==='fechamento_mensal_clientes'?'selected':'' ?>>Fechamento mensal de clientes</option><option value="cobranca_semanal_clientes" <?= $filtroRotina==='cobranca_semanal_clientes'?'selected':'' ?>>Cobranca semanal de clientes</option></select></div><div class="col-lg-3"><label class="form-label">Status</label><select name="status" class="form-select"><option value="">Todos</option><?php foreach(array_keys($resumoFila) as $statusFiltro): ?><option value="<?= htmlspecialchars($statusFiltro) ?>" <?= $filtroStatus===$statusFiltro?'selected':'' ?>><?= htmlspecialchars($statusFiltro) ?></option><?php endforeach; ?></select></div><div class="col-lg-3"><label class="form-label">Referencia</label><input name="referencia" class="form-control" value="<?= htmlspecialchars($filtroReferencia) ?>" placeholder="AAAA-MM"></div><div class="col-lg-2 d-flex gap-2"><button class="btn btn-primary flex-fill">Filtrar</button><a href="envios.php" class="btn btn-outline-secondary">Limpar</a></div></form></div><?php endif; ?>
    <div class="table-responsive"><table class="table table-sm table-hover align-middle mb-0"><thead><tr><?php if(!in_array($paginaOperacional,['fechamento','cobranca'],true)): ?><th>Rotina</th><?php endif; ?><th>Referencia</th><th>Destinatario</th><th>Telefone</th><th>Status</th><th>Tentativas</th><th class="text-end">Itens</th><th class="text-end">Valor</th><th>Aceito/entregue</th><th>Erro</th></tr></thead><tbody>
    <?php foreach($filaOperacional as $f): ?><tr><?php if(!in_array($paginaOperacional,['fechamento','cobranca'],true)): ?><td><?= htmlspecialchars($f['rotina_nome']) ?></td><?php endif; ?><td><?= htmlspecialchars($f['referencia']) ?></td><td><?= htmlspecialchars($f['destinatario_nome']) ?></td><td><?= htmlspecialchars($f['telefone'] ?: '-') ?></td><td><span class="badge <?= in_array($f['status'],['ACEITO','ENTREGUE'],true)?'text-bg-success':($f['status']==='ERRO'?'text-bg-danger':($f['status']==='AGUARDANDO'?'text-bg-primary':'text-bg-secondary')) ?>"><?= htmlspecialchars($f['status']) ?></span></td><td><?= (int)$f['tentativas'] ?>/3</td><td class="text-end"><?= (int)$f['quantidade_itens'] ?></td><td class="text-end">R$ <?= number_format((float)$f['valor'],2,',','.') ?></td><td><?= $f['entregue_em']?htmlspecialchars(date('d/m/Y H:i',strtotime($f['entregue_em']))):($f['aceito_em']?htmlspecialchars(date('d/m/Y H:i',strtotime($f['aceito_em']))):'-') ?></td><td><?= htmlspecialchars((string)($f['erro']??'')) ?></td></tr><?php endforeach; ?>
    <?php if(!$filaOperacional): ?><tr><td colspan="<?= in_array($paginaOperacional,['fechamento','cobranca'],true)?9:10 ?>" class="text-center text-muted py-3">Nenhum item na fila.</td></tr><?php endif; ?></tbody></table></div>
</div>
<?php endif; ?>
...
<?php if ($paginaOperacional === 'historico' || (in_array($paginaOperacional,['fechamento','cobranca'],true) && $secaoRotina === 'historico')): ?>
<div class="card shadow-sm mb-3"><div class="card-header"><h2 class="h5 mb-0"><?= in_array($paginaOperacional,['fechamento','cobranca'],true)?'Historico da rotina':'Historico das rotinas' ?></h2></div><div class="table-responsive"><table class="table table-striped table-sm mb-0"><thead><tr><?php if(!in_array($paginaOperacional,['fechamento','cobranca'],true)): ?><th>Rotina</th><?php endif; ?><th>Referencia</th><th>Destinatario</th><th>Status</th><th class="text-end">Itens</th><th class="text-end">Valor</th><th>Processado em</th><th>Erro</th></tr></thead><tbody>
<?php $temHistoricoFila=false; foreach($filaOperacional as $f): if(!in_array($f['status'],['ACEITO','ENTREGUE','ERRO','IGNORADO','CANCELADO'],true)) continue; $temHistoricoFila=true; ?><tr><?php if(!in_array($paginaOperacional,['fechamento','cobranca'],true)): ?><td><?= htmlspecialchars($f['rotina_nome']) ?></td><?php endif; ?><td><?= htmlspecialchars($f['referencia']) ?></td><td><?= htmlspecialchars($f['destinatario_nome']) ?></td><td><span class="badge <?= in_array($f['status'],['ACEITO','ENTREGUE'],true)?'text-bg-success':($f['status']==='ERRO'?'text-bg-danger':'text-bg-secondary') ?>"><?= htmlspecialchars($f['status']) ?></span></td><td class="text-end"><?= (int)$f['quantidade_itens'] ?></td><td class="text-end">R$ <?= number_format((float)$f['valor'],2,',','.') ?></td><td><?= htmlspecialchars(date('d/m/Y H:i',strtotime($f['atualizado_em'] ?: $f['criado_em']))) ?></td><td><?= htmlspecialchars((string)($f['erro']??'')) ?></td></tr><?php endforeach; ?>
...
<div class="card shadow-sm"><div class="card-header"><h2 class="h5 mb-0">Rotinas operacionais</h2></div><div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th>Rotina</th><th>Proxima execucao</th><th>Status</th><th class="text-end">Acao</th></tr></thead><tbody>
<tr><td><div class="fw-semibold">Fechamento mensal de clientes</div><div class="small text-muted">Envia a relacao de compras em aberto ate o encerramento da competencia.</div></td><td><?= $proximoEnvioAutomacao?htmlspecialchars($proximoEnvioAutomacao->format('d/m/Y H:i')):'Nao configurada' ?></td><td><span class="badge <?= ($config['fechamento_ativo']??'N')==='S'?'text-bg-success':'text-bg-secondary' ?>"><?= ($config['fechamento_ativo']??'N')==='S'?'Ativa':'Inativa' ?></span></td><td class="text-end"><a href="fechamento_mensal.php" class="btn btn-sm btn-outline-primary">Abrir</a></td></tr>
<tr><td><div class="fw-semibold">Cobranca semanal de clientes</div><div class="small text-muted">Envia todos os titulos em aberto aos clientes marcados que permanecem em atraso.</div></td><td><?= $proximaCobranca?htmlspecialchars($proximaCobranca->format('d/m/Y H:i')):'Nao configurada' ?></td><td><span class="badge <?= ($config['cobranca_ativo']??'N')==='S'?'text-bg-success':'text-bg-secondary' ?>"><?= ($config['cobranca_ativo']??'N')==='S'?'Ativa':'Inativa' ?></span></td><td class="text-end"><a href="cobranca_automatica.php" class="btn btn-sm btn-outline-primary">Abrir</a></td></tr>
</tbody></table></div></div>
<?php endif; ?>
