...
function crbInvertirTipomov($tipomov)
{
...
function crbBaixarTitulos(PDO $pdo, $empresaId, $usuarioId, array $dados)
{
    ...
    try {
        ...
        foreach ($titulos as $titulo) {
            ...
            if ($exigeContrap) {
                $movcontadorContrap = crbProximoMovcontador($pdo);
                $stmtBnc->execute([
                    $empresaId,
                    $movcontadorContrap,
                    $dataBaixa,
...
function crbMovimentoConciliadoExtrato(PDO $pdo, $empresaId, $movcontador)
{
    $stmt = $pdo->prepare("
        SELECT COUNT(*)
...
function crbExcluirBaixaTitulo(PDO $pdo, $empresaId, $usuarioId, $movcontador)
{
    ...
    if (crbMovimentoConciliadoExtrato($pdo, $empresaId, $movcontador)) {
    ...
    try {
        ...
        $pdo->prepare("
            ...
            SET deletado = 'S',
                data_delecao_firebird = NOW(),
                motivo_sync = 'BAIXA_CR_CONTRAPARTIDA_EXCLUIDA_MOVIMENTACAO_BAIXA',
                USERBNCALT = ?,
                DTALT = NOW(),
                REGSTAMP = NOW()
            WHERE EMPRESA = ?
