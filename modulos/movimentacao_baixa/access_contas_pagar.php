...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['acao'] ?? '') === 'amarrar_access') {
    try {
        ...
        header('Location: access_contas_pagar.php' . ($query ? '?' . $query . '&ok=vinculado' : '?ok=vinculado'));
        exit;
    } catch (Throwable $e) {
        $erro = $e->getMessage();
    }
...
if (($_GET['ok'] ?? '') === 'vinculado') {
    $mensagem = 'Pagamento amarrado ao Access.';
}
...
$camposData = ['DTVENC', 'DTPAGTO', 'DTEMISSAO', 'DTCOMPRA'];
...
if ($busca !== '') {
    $where[] = '(c.TITULO LIKE ? OR c.OBSERVACAO LIKE ? OR c.NUMDOCORIGEM LIKE ? OR c.CPCONTADOR = ?)';
    $params[] = '%' . $busca . '%';
    $params[] = '%' . $busca . '%';
    $params[] = '%' . $busca . '%';
    $params[] = (int)$busca;
...
$whereVinculo[] = "EXISTS (
    ...
    WHERE va.empresa_id = c.EMPRESA
      AND va.tabela_destino = 'armazem_cp001'
...
$stmtVinculados = $pdo->prepare("SELECT COUNT(*) FROM armazem_cp001 c WHERE {$whereVinculoSql}");
$stmtVinculados->execute($params);
$qtdVinculados = (int)$stmtVinculados->fetchColumn();

$limit = 100;
$stmt = $pdo->prepare("
    SELECT c.CPCONTADOR, c.DTVENC, c.DTPAGTO, c.DTEMISSAO, c.DTCOMPRA, c.VLRPARCELA, c.STATUS, c.TITULO, c.NUMDOCORIGEM,
           EXISTS (
                SELECT 1
                FROM mov_baixa_lancamentos_access va
                WHERE va.empresa_id = c.EMPRESA
    ...
    FROM armazem_cp001 c
    WHERE {$whereSql}
    ORDER BY c.{$dataSuper} DESC, c.CPCONTADOR DESC
    LIMIT {$limit}
");
$stmt->execute($params);
$linhas = $stmt->fetchAll(PDO::FETCH_ASSOC);
if ($buscarCandidatos) {
    $mapaCandidatos = acMapaCandidatosTitulo($pdo, $empresaId, $linhas, $dataAccess, $dataSuper, 'VLRPARCELA', 'CPCONTADOR');
    foreach ($linhas as $idx => $linha) {
        $linhas[$idx]['candidatos'] = ((int)($linha['vinculado_access'] ?? 0) === 1) ? [] : ($mapaCandidatos[(int)$linha['CPCONTADOR']] ?? []);
    }
} else {
    foreach ($linhas as $idx => $linha) {
    ...
    <div class="p-4 bg-white border rounded-2 shadow-sm">
        <div class="row align-items-center g-3">
            <div class="col-lg-8">
                ...
                <h1 class="h4 fw-bold mb-1">Contas a Pagar x Access</h1>
                <p class="text-muted mb-0">Procura CP001 no Access considerando valor igual e data aproximada.</p>
            </div>
        ...
        <form method="get" class="row g-3 align-items-end">
            ...
            <div class="col-md-2">
                <div class="form-check mt-4">
                    <input class="form-check-input" type="checkbox" name="data_vazia" value="1" id="data_vazia_cp" <?= $dataVazia ? 'checked' : '' ?>>
                    <label class="form-check-label" for="data_vazia_cp">DataPagamento vazia</label>
                </div>
            </div>
            ...
            <div class="col-md-12 d-flex gap-2">
                ...
                <a href="access_contas_pagar.php" class="btn btn-outline-secondary">Limpar</a>
            </div>
        </form>
    </div>
</section>
...
<section class="card ac-card">
    <div class="card-header">Titulos a pagar</div>
    <div class="card-body p-0"><div class="table-responsive">
        <table class="table table-sm table-hover ac-table mb-0">
            <thead><tr><th>CP</th><th>Data</th><th>Status</th><th>Valor</th><th>Titulo</th><th>Origem</th><th>Candidatos</th></tr></thead>
            <tbody>
            <?php foreach ($linhas as $linha): ?>
                <tr>
                    <td class="ac-code"><?= (int)$linha['CPCONTADOR'] ?></td>
                ...
                <?php if (is_array($linha['candidatos']) && count($linha['candidatos']) > 0): ?>
                    <tr class="ac-candidatos-row">
                        <td colspan="7">
                            <details>
                                <summary>Ver candidatos Access para o CP <?= (int)$linha['CPCONTADOR'] ?></summary>
                                <div class="mt-2">
                                    <?= acRenderCandidatos($linha['candidatos'], 'cp', (int)$linha['CPCONTADOR']) ?>
                                </div>
                            </details>
                        </td>
                    </tr>
