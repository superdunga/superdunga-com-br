...
@app.route("/dados/bnc001_historico", methods=["GET"])
def dados_bnc001_historico():
    try:
        data_inicio = request.args.get("data_inicio", default="1900-01-01", type=str)
        data_fim = request.args.get("data_fim", default=None, type=str)
        limit = request.args.get("limit", default=5000, type=int)
        offset = request.args.get("offset", default=0, type=int)
        mov_inicio = request.args.get("mov_inicio", default=None, type=int)
        mov_fim = request.args.get("mov_fim", default=None, type=int)
        ...
        if data_fim:
            filtros.append("DTMOV <= ?")
            params.append(data_fim)

        if mov_inicio is not None:
            filtros.append("MOVCONTADOR >= ?")
            params.append(mov_inicio)

        if mov_fim is not None:
            filtros.append("MOVCONTADOR <= ?")
            params.append(mov_fim)
        ...
        cursor.execute(f"""
            ...
            FROM BNC001
            WHERE {where_sql}
            ORDER BY DTMOV, EMPRESA, MOVCONTADOR
            ROWS {inicio} TO {fim}
        """, tuple(params))
    ...
    try:
        ...
        if isinstance(dados, list):
            ...
            for item in dados:
                ...
                if campos:
                    valores.extend([crcontador, empresa])
