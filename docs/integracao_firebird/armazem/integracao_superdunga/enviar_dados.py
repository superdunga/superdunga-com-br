...
def processar_bnc001_historico():
    data_inicio = sys.argv[2] if len(sys.argv) > 2 else "1900-01-01"
    data_fim = sys.argv[3] if len(sys.argv) > 3 else "2024-12-31"
    tamanho_lote = int(sys.argv[4]) if len(sys.argv) > 4 else 5000
    offset = 0
    total_enviado = 0

    print("\nProcessando BNC001 historico...")
    print(f"Periodo: {data_inicio} ate {data_fim}")
    print(f"Tamanho do lote: {tamanho_lote}")

    url_api = "http://127.0.0.1:5000/dados/bnc001_historico"
    ...
    while True:
        ...
        lote_numero = offset // tamanho_lote + 1
        print(f"Enviando lote historico {lote_numero} com {len(dados)} registros...")
    ...
    registrar_log(
        ...
        total_enviado,
        "OK",
        f"Carga historica enviada de {data_inicio} ate {data_fim}"
    )
    print(f"\nBNC001 historico finalizado. Total enviado: {total_enviado}")
        ...
        if len(dados) > 0:
            ...
            if tabela_php == "est007":
                ...
                for i in range(0, len(dados), tamanho_lote):
                    ...
                    envio = requests.post(url_php, params=params_site({"tabela": tabela_php}), json=lote, timeout=600)
                    envio.raise_for_status()
                ...
                envio = requests.post(url_php, params=params_site({"tabela": tabela_php}), json=dados, timeout=600)
                envio.raise_for_status()
...
def enviar_ativos(nome, url_api, tabela_php, tamanho_lote=1000, params_api=None, params_php=None):
    try:
        ...
        params_php = params_site(params_php)
...
if MODO in ["bnc001_historico", "historico_bnc001"]:
    try:
        processar_bnc001_historico()
    except Exception as e:
        erro = str(e)
...
try:
    ...
    if len(dados) > 0:
        ...
        envio = requests.post(url_php, params=params_site({"tabela": "bnc001"}), json=dados, timeout=600)
        envio.raise_for_status()
