...
def atualizar_firebird(con, firebird_empresa, registros):
    cursor = con.cursor()
    resultados = []
        ...
        try:
            ...
            cursor.execute(
                ...
                WHERE EMPRESA = ?
                  AND CODPRODUTO = ?
                """,
                (qtd_nova, firebird_empresa, codproduto),
            )
            resultados.append({"id": item_id, "status": "ok"})
...
def main():
    ...
    parser.add_argument("--firebird-empresa", type=int, default=None)
    ...
    print(f"Empresa Firebird usada no EST004: {firebird_empresa}")
    ...
    try:
        resultados = atualizar_firebird(con, firebird_empresa, pendencias)
    finally:
        con.close()
